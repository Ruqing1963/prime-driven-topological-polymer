...
\date{\normalsize Version~2.0 (mechanism-falsification update)\\[3pt]\today}
...
$\xi \approx 3.5\times10^{2}$ gaps (${\approx}30\%$ of the sequence) and
falsifying the local-cluster (\emph{topological-trap}) hypothesis of version~1.
\end{abstract}
...
annealing protocol of Sec.~\ref{sec:model} (Table~\ref{tab:params}), so that each
surrogate is directly comparable to the true and S1 ensembles of
Sec.~\ref{sec:decomp}.
...
$N=10^{4}$. The order responsible for the topological imprint is, in this precise
sense, \emph{macroscopic}: it resides not in the neighbourhood of any individual
prime but across a finite fraction of the entire progression sampled. We note that
$\xi$ exceeds the few-lag range over which prime-gap pair-clustering is
...
\caption{The S4/S5 scissors at $N=10^{4}$, $n=40$ per arm. Spearman $\rho$
...
$\xi \approx 3.5\times10^{2}$ gaps---a scale spanning ${\approx}30\%$ of the sequence.
The correlation scale responsible for the signal, identified as the most immediate
open question in version~1, is thus now measured. Beyond the specific case of primes,
...
ensembles and the scissors figure.
